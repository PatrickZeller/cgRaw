...
def selection_filter(node):
    ...
    coloraovs = set(
        [item.split(".")[0] for item in coloraovs if re.search("_col_", item)]
    )
...
def update_isolate(node):
    ...
    with node:
        ...
        [
            node.knob(k).setValue(" + ".join([i + expressions[k] for i in selection]))
...
def create_layer(node):
    ...
    nuke.Layer(layer, [layer + ".red", layer + ".green", layer + ".blue"])
    return layer
